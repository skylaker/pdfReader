...
public class CircleAnnotHandler implements AnnotHandler {
    ...
    public void onColorValueChanged(int color) {
        ...
        try {
            if (annot != null && ToolUtil.getCurrentAnnotHandler(uiExtensionsManager) == this && color != annot.getBorderColor()) {
                modifyAnnot(annot.getPage().getIndex(), annot, annot.getRect(), color, (int)(((Circle)annot).getOpacity() * 255f), annot.getBorderInfo().getWidth(), annot.getContent(), false,false, null);
            }
    ...
    public void onOpacityValueChanged(int opacity) {
        ...
        UIExtensionsManager uiExtensionsManager = (UIExtensionsManager) mPdfViewCtrl.getUIExtensionsManager();
        try {
            if (annot != null && ToolUtil.getCurrentAnnotHandler(uiExtensionsManager) == this && opacity != (int)(((Circle)annot).getOpacity() * 255f)) {
    ...
    public void onAnnotDeselected(Annot annot, boolean needInvalid) {
        ...
        try {
            ...
            if (needInvalid && mIsModify) {
                ...
                if (mTempLastColor == (int) annot.getBorderColor() && mTempLastLineWidth == annot.getBorderInfo().getWidth() && mTempLastBBox.equals(annot.getRect()) && mTempLastOpacity == (int) (((Circle)annot).getOpacity() * 255f)) {
                    modifyAnnot(page.getIndex(), annot, annot.getRect(), (int) annot.getBorderColor(), (int) (((Circle)annot).getOpacity() * 255f), annot.getBorderInfo().getWidth(), annot.getContent(),false, false, null);
                } else {
                    modifyAnnot(page.getIndex(), annot, annot.getRect(), (int) annot.getBorderColor(), (int) (((Circle)annot).getOpacity() * 255f), annot.getBorderInfo().getWidth(), annot.getContent(),true, true, null);
                }
            } else if (mIsModify) {
    ...
    private void preparePropertyBar() {
        ...
        try {
            ...
            int opacity = AppDmUtil.opacity255To100((int) (((Circle) DocumentManager.getInstance(mPdfViewCtrl).getCurrentAnnot()).getOpacity() * 255f + 0.5f));
    ...
    public void addAnnot(final int pageIndex, AnnotContent contentSupplier, final boolean addUndo, final Event.Callback result) {
        ...
        try {
            ...
            final Annot annot = page.addAnnot(Annot.e_annotSquare, contentSupplier.getBBox());
