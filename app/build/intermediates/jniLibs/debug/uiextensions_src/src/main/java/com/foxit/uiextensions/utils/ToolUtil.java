...
public class ToolUtil {
    ...
    public static void registerAnnotHandler(UIExtensionsManager UIExtensionsManager, AnnotHandler handler) {
        Class<UIExtensionsManager> clazz = UIExtensionsManager.class;
        try {

            Method method = clazz.getDeclaredMethod("registerAnnotHandler", AnnotHandler.class);
            method.setAccessible(true);
            try {
                method.invoke(UIExtensionsManager, handler);
            } catch (IllegalArgumentException e) {
                e.printStackTrace();
            } catch (IllegalAccessException e) {
                e.printStackTrace();
    ...
    public static void unregisterAnnotHandler(UIExtensionsManager UIExtensionsManager, AnnotHandler handler) {
        Class<UIExtensionsManager> clazz = UIExtensionsManager.class;
        try {

            Method method = clazz.getDeclaredMethod("unregisterAnnotHandler", AnnotHandler.class);
            method.setAccessible(true);
            try {
                method.invoke(UIExtensionsManager, handler);
            } catch (IllegalArgumentException e) {
