...
public abstract class AbstractAnnotHandler implements AnnotHandler, PropertyBar.PropertyChangeListener {
            ...
            public void result(Event event, boolean success) {
                if (success) {
                    try {
                        ...
                        if (addUndo) {
                            DocumentManager.getInstance(mPdfViewCtrl).addUndoItem(modifyEvent.mUndoItem);
                        }
    ...
    protected void handleRemoveAnnot(Annot annot, final EditAnnotEvent deleteEvent, final boolean addUndo, final IAnnotTaskResult<PDFPage, Void, Void> result) {
        try {
            ...
            EditAnnotTask task = new EditAnnotTask(deleteEvent, new Event.Callback() {
                ...
                public void result(Event event, boolean success) {
                    if (success) {
                        if (addUndo) {
                            DocumentManager.getInstance(mPdfViewCtrl).addUndoItem(deleteEvent.mUndoItem);
                        }
                        if (mPdfViewCtrl.isPageVisible(pageIndex)) {
                            pvRect.roundOut(tv_rect1);
                            mPdfViewCtrl.refresh(pageIndex, tv_rect1);
