...
public class DefaultAnnotHandler extends AbstractAnnotHandler {
	...
	public DefaultAnnotHandler(Context context, PDFViewCtrl pdfViewCtrl) {
		super(context, pdfViewCtrl, Annot.e_annotUnknownType);
	...
	public void addAnnot(int pageIndex, final AnnotContent content, boolean addUndo, final Event.Callback result) {
		try {
			...
			addAnnot(pageIndex, annot, undoItem, addUndo,
	...
	protected void addAnnot(int pageIndex, Annot annot, DefaultAnnotAddUndoItem undoItem, boolean addUndo, IAnnotTaskResult<PDFPage, Annot, Void> result) {
		DefaultAnnotEvent event = new DefaultAnnotEvent(EditAnnotEvent.EVENTTYPE_ADD, undoItem, annot, mPdfViewCtrl);
...
class DefaultAnnotAddUndoItem extends DefaultAnnotUndoItem {
	...
	public boolean redo() {
		try {
			PDFPage page = mPdfViewCtrl.getDoc().getPage(mPageIndex);
			Annot annot = page.addAnnot(mType, mBBox);
