		...
		setListener(new DialogListener() {
			...
			public void onResult(long btType) {
				if (btType == UIMatchDialog.DIALOG_OK) {
					...
					try {
						bModified = mPdfViewCtrl.getDoc().isModified();
					} catch (PDFException e) {
						e.printStackTrace();
					}
					if((bModified) || (DocumentManager.getInstance(mPdfViewCtrl).canSaveAsFile() && !DocumentManager.getInstance(mPdfViewCtrl).canModifyFile())) {//if document is modified, show dialog to confirm save document first, then setting password
						UIEncryptionDialogFragment newFragment = null;
						FragmentManager fm = ((FragmentActivity)mContext).getSupportFragmentManager();
						newFragment = (UIEncryptionDialogFragment)fm.findFragmentByTag(DIALOGTAG);
						if (newFragment == null) {
						...
						newFragment.setEncryptionDialogEventListener(new UIEncryptionDialogFragment.UIEncryptionDialogEventListener() {
							...
							public void onConfirmed(boolean encrypt) {
								...
								if (DocumentManager.getInstance(mPdfViewCtrl).canSaveAsFile() && !DocumentManager.getInstance(mPdfViewCtrl).canModifyFile()) {
